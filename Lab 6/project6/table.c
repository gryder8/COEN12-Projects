...
# include <assert.h>
# include <stdbool.h>
# include "set.h" //ignore error here
...
//Worst: O(n)
//partition the array, return the new pivot point
int partition(SET *sp, void **arr, int low, int high) { //set simply provides the compare functionality
    assert(sp != NULL);
    void *pvt = arr[high];
    int i = low - 1; //index of the smaller element, also the pivot so far
    void *tmp;
    int j;
    for (j = low; j < high; j++) { //go between low and high
        if ((*sp->compare)(arr[j], pvt) <= 0) { //use set compare due to generic
            i++;     //increment smallest                          
    ...
    tmp = arr[i+1]; //swap the index + 1 with the high (we have a new high)
    arr[i+1] = arr[high];
    arr[high] = tmp;

    return (i+1); //return the new high to pivot around
}

//Worst Case: O(n^2)
//Sort using recursive partitioning of the array
void quicksort(SET *sp, void **arr, int low, int high) {
    assert(sp != NULL);
    if (low < high) {
        int p = partition(sp, arr, low, high);
        quicksort(sp, arr, low, p-1);
        quicksort(sp, arr, p+1, high);
    }
}
 ...
 *
 * Complexity:	O(n^2) [same as quicksort]
 * 
 * Description:	Allocate and return an array of sorted elements in the set pointed
 *		to by SP by using quicksort.
 */

void *getElements(SET *sp)
{
    //use same logic, just sort the end result
    assert(sp != NULL);
    int i, j;
    void **sortedArray = malloc(sizeof(void *) * sp->count); //same malloc
    assert(sortedArray != NULL);

    for (i = 0, j = 0; i < sp->length; i ++) {
	if (sp->flags[i] == FILLED)
	    sortedArray[j ++] = sp->data[i];
    }
    quicksort(sp, sortedArray, 0, sp->count-1); //sort the array from  0 to the end
